...
class Controller:
    ...
    def register_handlers(self):
        ...
        def send_welcome(message):
            self.bot.reply_to(
                message,
                "Добро пожаловать в Todo-бота! Используйте команды: /add(добавить задачу), /list(посмотреть задачи), /delete(удалить задачу), /update(обновить статус задачи).",
            )

        @self.bot.message_handler(commands=["add"])
        def add_task(message):
            msg = self.bot.reply_to(message, "Введите задачу:")
    ...
    def _update_task_handler(self, message, user_id):
        try:
            ...
            msg = self.bot.reply_to(
                message, "Введите новый текст задачи или статус задачи (true/false):"
            )
            self.bot.register_next_step_handler(
